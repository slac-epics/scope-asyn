...
/* drvMDO.cpp
 * Asyn driver to control Tektronix MDO3x series scopes.  
 * This is a subclass of the drvScope base class.
 * 04/14/2023, md
 *---------------------------------------------------------------------------*/
...
const char* TmDlyEnCmnd    = "HOR:DEL:MOD";
const char* TmSclCmnd      = "HOR:SCA";
const char* TrigPosCmnd    = "HOR:POS";
...
const char* InitCmnd       = "*CLS; :DAT:ENC RIB; :HOR:RECORDL 1000; :HEAD OFF; :VERB ON; DAT:WID 1; :DAT:STAR 1; :DAT:STOP 1000; HOR:DEL:MOD 1";
...
drvMDO::drvMDO(const char* port, const char* udp):
            drvScope(port, udp),
             _num_meas(4) {
/*------------------------------------------------------------------------------
 * Constructor for the drvMDO class. Calls the base class constructor, which
...
void drvMDO::_get_hs_params(double hs,int* x0,int* np){
/*-----------------------------------------------------------------------------
...
void drvMDO::getHSParams(double hs, int* x0, int* np){
    _get_hs_params(hs, x0, np);
}


asynStatus drvMDO::trigState(){
/*-----------------------------------------------------------------------------
...
const char** drvMDO::getCmndList(int cix, uint* ni) {
/*-----------------------------------------------------------------------------
...
const char* drvMDO::getCommand(int cix) {
/*-----------------------------------------------------------------------------
...
int drvMDO::_wfPreamble(char* p, int* ln, int* nb, double* ym, double* yz, double* yo) {
/*-----------------------------------------------------------------------------
 ...
 *---------------------------------------------------------------------------*/
    ...
    char chs[16], enc[20], bfmt[20], bord[20], ids[80], ptfm[20], ptord[20], xunt[20], yunt[20], char1[20], char2[20];
    double xinc, xzr, ymult, yzr, yof, d1, d2, d3;

    n = sscanf(p, "%d;%d;%19[^;];%19[^;];%19[^;];%79[^;];%d;%19[^;];%19[^;];"
          "%19[^;];%lg;%lg;%d;%19[^;];%lg;%lg;%lg;%19[^;];%19[^;];%lg;%lg;%lg;%n",
          &nbyt, &nbit, enc, bfmt, bord, ids, &len, ptfm, ptord,
          xunt, &xinc, &xzr, &ptof, yunt, &ymult, &yof, &yzr, char1, char2, &d1, &d2, &d3, &i);
    
    //printf("_wfPreamble: yunt=%s, ymult=%g, yof=%g, yzr=%g, i=%d\n", yunt, ymult, yof, yzr, i);
...
void drvMDO::getWaveform(int ch) {
/*-----------------------------------------------------------------------------
...
void drvMDO::getMeasurements(int pollCount) {
/*-----------------------------------------------------------------------------
 *---------------------------------------------------------------------------*/
    char cmnd[32]; 
...
void drvMDO::timeDelayStr(int m, int uix) {
/*-----------------------------------------------------------------------------
...
void drvMDO::setTimePerDiv(double v) {
/*-----------------------------------------------------------------------------
...
void drvMDO::_setTimePerDiv(uint vix, uint uix) {
/*-----------------------------------------------------------------------------
...
asynStatus drvMDO::getCmnds(int ix, int addr){
/*-----------------------------------------------------------------------------
...
asynStatus drvMDO::putIntCmnds(int ix, int addr, int v) {
/*-----------------------------------------------------------------------------
...
asynStatus drvMDO::putFltCmnds(int ix, int addr, float v) {
/*-----------------------------------------------------------------------------
...
asynStatus drvMDO::writeInt32(asynUser* pau, epicsInt32 v) {
/*-----------------------------------------------------------------------------
...
asynStatus drvMDO::writeFloat64(asynUser* pau, epicsFloat64 v){
/*-----------------------------------------------------------------------------
 * This method overrides the virtual method in asynPortDriver.  Here we service
 * all write requests comming from EPICS records.
 * Parameters:
