 ...
 *---------------------------------------------------------------------------*/
  int len;
  strncpy( _mbuf,putil->getTime(),MSGNB); _mbuf[MSGNB-1]=0;
 ...
 *---------------------------------------------------------------------------*/
  const char* cmnd=getCommand(cix);
  if(!cmnd) return(asynError);
  uint ni; const char** list=getCmndList( cix,&ni);
  if(!list) return(asynError);
 ...
 *---------------------------------------------------------------------------*/
  ...
  if(!strchr( str,'?')) strcat( str,"?");
  stat=_wtrd( str,strlen(str),_rbuf,DBUF_LEN);
  if(stat!=asynSuccess) return(asynError);
  val=_find(_rbuf,list,ni);
 ...
 *---------------------------------------------------------------------------*/
  ...
  switch( jx){
    ...
    case ixSiIpAddr:	_getIpAddr(); break;
    case ixAoTimDly:	getFloat( jx,ix);
			getDoubleParam( ix,&dv);
			timeDelayStr(dv);
			break;
    case ixAiTimDiv:	getFloat( jx,ix);
			getDoubleParam( ix,&dv);
			setTimePerDiv(dv);
			break;
    case ixAoTrPos:	getFloat( jx,ix); break;
    case ixAoTrLev:	getFloat( jx,ix);
			getTrigLevl();
			break;
    case ixAoTrHOff:	getFloat( jx,ix); break;
    case ixSiWfFmt:	getString( jx,ix); break;
    case ixBoChOn:	_getChanOn( ch);
			break;
    case ixAoChPos:	getChanPos( addr);
			getDoubleParam( addr,ix,&dv);
			if(addr==_chSel) _setPosSlider(dv);
			break;
    case ixBoChImp:	getBinaryCh( jx,ch,ix); break;
    case ixMbboChCpl:	getBinaryCh( jx,ch,ix); break;
    case ixAoChScl:	getFloatCh( jx,ch,ix); break;
    case ixLoWfNpts:	getInt( jx,ix); break;
    case ixLoWfStart:	getInt( jx,ix); break;
    case ixLoWfStop:	getInt( jx,ix); break;
    case ixLiEsr:	getInt( jx,ix); break;
    case ixLoEse:	getInt( jx,ix); break;
    case ixLiStb:	getInt( jx,ix); break;
 ...
 *---------------------------------------------------------------------------*/
  ...
  switch( jx){
    ...
    case ixBoSave:	if(v) saveConfig(); break;
    case ixBoRestore:	if(v) restoreConfig(); break;
    case ixBoChOn:	if(!pcmd) break;
			sprintf( cmnd,pcmd,addr+1);
			if(v) strcat(cmnd," ON");
			...
			_selectChannel();
			break;
    case ixBoChImp:	setIntegerParam( addr,ix,v);
			setBinaryCh( v,addr,jx);
			break;
    case ixMbboChCpl:	setIntegerParam( addr,ix,v);
			setBinaryCh( v,addr,jx);
			break;
    case ixBoTimDlySt:	if(!pcmd) break;
			sprintf( cmnd,"%s %d",pcmd,v);
			command( cmnd);
			setIntegerParam( addr,ix,v);
			break;
    case ixBoGetWf:	getWaveform( addr);
			break;
    case ixBoStop:
    ...
    case ixBoReset:	if(!pcmd) break;
			command(pcmd); break;
    case ixBoInit:	if(!pcmd) break;
			command(pcmd); break;
    case ixLoWfNpts:	setInt( jx,v,ix); break;
    case ixLoWfStart:	setInt( jx,v,ix); break;
    case ixLoWfStop:	setInt( jx,v,ix); break;
    case ixLoEse:	setInt( jx,v,ix); break;
    case ixBoCls:	if(!pcmd) break;
			command(pcmd); break;
    case ixBoAPed:	_doPeds[addr]=1; break;
    case ixBoChSel:	_selectChan(addr); break;
    case ixLoChPos:	_posInProg=1;
			_chPosTimer->start(0.2);
			_chPos=v/100.0;
			break;
    case ixLoTrLev:	setTrigLevl( v); break;
 ...
 *---------------------------------------------------------------------------*/
  ...
  jx=ix-_firstix;
  switch( jx){
    case ixBoAnal:	getIntegerParam( addr,_boChOn,&on);
 ...
 *---------------------------------------------------------------------------*/
  ...
  switch( ix){
    case ixAoPTMO:	_pollT=v;
			break;
    default:		putInMessgQ( enPutFlt,ix,addr,0,fv); break;
  }
  return(stat);
}
 ...
 *---------------------------------------------------------------------------*/
  static int first[]={1,1,1,1}; int i=ch-1,v; double dv;
 ...
 *--------------------------------------------------------------------------*/
  int i,ch; static int firsttime=1; double dv;
  ...
  getFloat( ixAoTimDly,_aoTimDly); getDoubleParam( _aoTimDly,&dv);
  timeDelayStr(dv);
  getInt( ixBoTimDlySt,_boTimDlySt);
  getFloat( ixAoTrPos,_aoTrPos);
  getFloat( ixAoTrLev,_aoTrLev); getTrigLevl();
